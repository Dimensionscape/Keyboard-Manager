...
class KeyboardManager
{
	...
	**/
	...
	{
		if (isInitialized) throw new Error("The KeyboardManager is already running and it must be disposed before calling start() again.");
		
		KeyboardManager._stage = stage;
		
		_keysDown = new IntMap();
		_keysUp = new IntMap();		
		_keysPressed = new IntBoolMap();
	...
	**/
	public static function resume():Void{
		if (!isInitialized) throw new Error("You cannot resume a KeyboardManager that has been disposed or not started previously");
		if (isRunning) return;
		_stage.addEventListener(KeyboardEvent.KEY_DOWN, _onKeyDown);
		_stage.addEventListener(KeyboardEvent.KEY_UP, _onKeyUp);
	...
	**/
	...
	{
		if (!isInitialized) throw new Error("A KeyboardManager must be initialized in order to access this method");
		
		return _keysPressed.get(keyCode);
	}
	...
	**/
	...
	{
		if(keysDown != null) _keysDown.remove(keyCode);
	}
	...
	**/
	...
	{
		if(keysUp != null) _keysUp.remove(keyCode);
	}
	...
	**/
	...
	{
		if(keysDown != null) _keysDown.clear();
	}
	...
	**/
	...
	{
		if(keysUp != null) _keysUp.clear();
	}

	private static function _onKeyDown(e:KeyboardEvent)
	{
		var keyCode:Int = e.keyCode;
		_keysPressed.set(e.keyCode, true);
	...
	{
		var keyCode:Int = e.keyCode;
		_keysPressed.set(keyCode, false);
